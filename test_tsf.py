...
def test(model, dataset, history_len, pred_len, batch_size):
    ...
    with torch.no_grad():
        ...
        for idx in range(history_len, len(dataset.test_df)-pred_len, batch_size):
                ...
                col = 'Price'
                past_ts,date_features_vector, future_date_features_vector, future_ts, related_ts= dataset.get_io_i('test', col, idx, history_len, pred_len)
                ...
                for i in range(batch_size):
                    if idx+i>=len(dataset.test_df)-pred_len:
                        break
                    col = 'Price'
                    past_ts,date_features_vector, future_date_features_vector, future_ts, related_ts= dataset.get_io_i('test', col, idx+i, history_len, pred_len)
...
if __name__ == '__main__':
    ...
    path = '/mnt/nas/kunalchhabra/TIDE_Models/tsf_global_encoder_bn__epoch_10'
